    ...
    if (userExist && userExist.isAdmin == false) {
        ...
        if (isPasswordMatched) {
            if (userExist.isActive == true) {
                ...
                return res.status(200).send({response: "Success", message: 'Success', name: userExist.name, token: token })
            } else {
                return res.status(501).send({response: "Error", message: 'Active su cuenta primero, revise su correo electronico'})
            }
        ...
        if (userExist && userExist.isAdmin == true) {
            ...
            return res.status(500).send({response: "Error", message: "No se pudo encontrar el usuario" })
        }
    }
}
